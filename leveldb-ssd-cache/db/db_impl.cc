...
Status DBImpl::RecoverLogFile(uint64_t log_number,
  ...
  while (reader.ReadRecord(&record, &scratch) &&
    ...
    status = WriteBatchInternal::InsertInto(&batch, mem, nullptr, nullptr, nullptr);
    MaybeIgnoreError(&status);
    if (!status.ok()) {
      break;
    }
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  {
    mutex_.Unlock();
    StopWatch sw(env_, statistics_, MEMTABLE_FLUSH_MICROS);
...
Status DBImpl::Get(const ReadOptions& options,
  ...
  //uint64_t s1 = env_->NowMicros();
  MutexLock l(&mutex_);
  //statistics_->measureTime(GET_LOCK_MICROS, env_->NowMicros() - s1);
  ...
  if (imm != NULL) imm->Ref();
  current->Ref();
  //statistics_->measureTime(GET_REF_STUFF_MICROS, env_->NowMicros() - s2);
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* my_batch) {
  ...
  if (status.ok() && my_batch != NULL) {  // NULL batch is for compactions
    ...
    {
      StopWatch sw(env_, statistics_, BUILD_BATCH_GROUP_MICROS);
      updates = BuildBatchGroup(&last_writer);
    ...
    // into mem_.
    {
      ...
      {
        StopWatch sw(env_, statistics_, WAL_APPEND_MICROS);
...
Status DBImpl::MakeRoomForWrite(bool force) {
  ...
  while (true) {
    if (!bg_error_.ok()) {
      ...
      RecordTick(statistics_, NUM_THROTTLED_WRITES);
      mutex_.Unlock();
      ...
      RecordTick(statistics_, NUM_MEMTABLE_WAIT_WRITES);
      StopWatch sw(env_, statistics_, MEMTABLE_WAIT_MICROS);
      bg_cv_.Wait();
    } else if (versions_->NumLevelFiles(0) >= config::kL0_StopWritesTrigger) {
      // There are too many level-0 files.
      Log(options_.info_log, "Too many L0 files; waiting...\n");
      RecordTick(statistics_, NUM_STALLED_WRITES);
      StopWatch sw(env_, statistics_, STALLED_WRITE_MICROS);
      bg_cv_.Wait();
    } else {
